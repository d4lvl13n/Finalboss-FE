...
// and (where relevant) the live list of handhelds that have the feature.
...
import { GLOSSARY, getTerm } from '@/app/lib/handhelds/glossary';
import { getCategory, categoryHandhelds, categoryChipLabel } from '@/app/lib/handhelds/categories';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    path: `/handhelds/glossary/${term.slug}`,
  });
}
...
export default function GlossaryTermPage({ params }: Props) {
  ...
  const handhelds = category ? categoryHandhelds(category).slice(0, 8) : [];
  ...
  const graph = [
    {
      ...
      name: term.term,
      description: term.short,
      url: absoluteUrl(`/handhelds/glossary/${term.slug}`),
      inDefinedTermSet: absoluteUrl('/handhelds/glossary'),
    },
    {
      '@context': 'https://schema.org',
      '@type': 'BreadcrumbList',
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: 'Glossary', item: absoluteUrl('/handhelds/glossary') },
        { '@type': 'ListItem', position: 3, name: term.term, item: absoluteUrl(`/handhelds/glossary/${term.slug}`) },
      ],
    },
  ];
      ...
      <main className="min-h-screen bg-gray-950 text-gray-100">
        <div className="mx-auto max-w-3xl px-4 pb-16 pt-24">
          <nav className="mb-6 text-sm text-gray-500">
            ...
            <Link href="/handhelds/glossary" className="hover:text-gray-300">
              Glossary
            </Link>
            <span className="px-2">/</span>
          ...
          <article>
            ...
            {category && handhelds.length ? (
              <section className="mt-12">
                <div className="mb-4 flex items-baseline justify-between gap-3">
                  <h2 className="text-xl font-bold text-white">Handhelds with {term.term}</h2>
                  <Link
                    href={`/handhelds/best/${category.slug}`}
