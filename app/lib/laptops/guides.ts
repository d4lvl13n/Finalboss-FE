...
// content/laptop-guides/*.md (frontmatter = title/description/picks/faq, body =
// editorial prose). Picks reference families by slug; this loader resolves them
...
const DIR = path.join(process.cwd(), 'content/laptop-guides');
...
function parseFile(file: string): Guide {
  ...
  const picks: GuidePick[] = rawPicks.map((p) => {
    const family = p.slug ? getFamily(p.slug) : null;
    if (!family) {
      throw new Error(`[guides] ${file}: unknown pick slug "${p.slug}" — fix the .md frontmatter`);
    }
    return { slug: family.slug, note: String(p.note ?? ''), family };
  });
