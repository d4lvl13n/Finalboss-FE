            ...
            <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="p-6 rounded-2xl bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-sm border border-white/10 hover:from-white/10 hover:to-white/15 transition-all duration-300 group">
                ...
                <ul className="space-y-3">
                  {[
                    { href: '/authors', label: t('footer.ourTeam') },
                    { href: '/write-for-us', label: t('nav.writeForUs') },
                    { href: '/about', label: t('nav.aboutUs') },
                    { href: '/contact', label: t('nav.contact') },
                    { href: '/privacy-policy', label: t('pages.privacy.heading') },
              ...
              <div className="p-6 rounded-2xl bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-sm border border-white/10 hover:from-white/10 hover:to-white/15 transition-all duration-300 group">
                <div className="flex items-center mb-4">
                  ...
                  <h3 className="text-white font-bold text-lg">{t('footer.categories')}</h3>
                </div>
                <ul className="space-y-3">
                  {[
                    { href: '/articles', label: t('nav.news'), icon: FaNewspaper },
                    { href: '/reviews', label: t('nav.reviews'), icon: FaStar },
                    { href: '/guides', label: t('nav.guides'), icon: FaBookOpen },
                    { href: '/games', label: t('nav.games'), icon: FaGamepad },
                    { href: '/videos', label: t('nav.videos'), icon: FaVideo },
              ...
              <div className="p-6 rounded-2xl bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-sm border border-white/10 hover:from-white/10 hover:to-white/15 transition-all duration-300 group">
                <div className="flex items-center mb-4">
                  ...
                  <h3 className="text-white font-bold text-lg">{t('footer.connect')}</h3>
                </div>

                {/* Social Media */}
                <div className="mb-6">
                  <h4 className="text-blue-400 font-semibold text-sm uppercase tracking-wider mb-3">{t('footer.followUs')}</h4>
                  <div className="flex flex-wrap gap-3">
                    {[
                      { href: siteConfig.socialLinks.twitter, icon: FaTwitter, color: "hover:text-blue-400" },
                      { href: siteConfig.socialLinks.facebook, icon: FaFacebookF, color: "hover:text-blue-600" },
                      { href: siteConfig.socialLinks.instagram, icon: FaInstagram, color: "hover:text-pink-400" },
                      { href: siteConfig.socialLinks.youtube, icon: FaYoutube, color: "hover:text-red-500" }
                    ].map((social, index) => (
                      <a
                        key={index}
                        href={social.href}
                ...
                {/* Contact Info */}
                <div>
                  <h4 className="text-green-400 font-semibold text-sm uppercase tracking-wider mb-3">{t('footer.contactLabel')}</h4>
                  <div className="space-y-2 text-sm text-gray-300">
                    <div className="flex items-center">
                      ...
                      <button
                        ...
                        onMouseEnter={(e) => {
                          e.currentTarget.title = `Click to send email to ${siteConfig.contactEmail}`;
                        }}
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="text-center md:text-left">
                <p className="text-gray-400 text-sm">
                  {t('footer.copyright', { year: new Date().getFullYear(), name: siteConfig.name })}
                </p>
              ...
              <div className="flex items-center space-x-3">
                <div className="px-4 py-2 bg-gradient-to-r from-yellow-400/10 to-purple-500/10 border border-yellow-400/20 rounded-full">
                  <span className="text-xs text-yellow-400 font-semibold tracking-wider">{t('footer.gamingFirst')}</span>
                </div>
